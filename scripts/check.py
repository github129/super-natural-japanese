...
def check_keigo(text, f, register):
    ...
    # 数量を述べるだけの「〜となります」: 「見積金額は1,250,000円となります」
    # 変化や結果を述べる「4月から有料となります」は対象外(直前が数量でない)
    scan(text, r"[0-9０-９][0-9０-９,，.]*(円|名|人|件|個|台|枚|回|日|時|分|割|%|％)となります", f, "INFO", "敬語",
         "数量を述べるだけの「〜となります」", fix="〜です")
